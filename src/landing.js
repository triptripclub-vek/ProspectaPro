...
.adviceCard{min-height:545px;position:relative;overflow:hidden;background:#20242a}.adviceCard .heroImage{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:72% center;display:block;z-index:0}.adviceCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.12),rgba(0,0,0,.72));z-index:1}.adviceContent{position:absolute;inset:0;padding:32px 28px;color:#fff;display:flex;flex-direction:column;justify-content:flex-start;z-index:2}.adviceContent h1{font-size:clamp(34px,3.4vw,49px);line-height:1.04;letter-spacing:-.055em;margin:0 0 13px;max-width:450px}.adviceContent h2{font-size:23px;line-height:1.18;margin:0 0 22px}.adviceContent p{font-size:16px;line-height:1.52;margin:0 0 12px;max-width:430px}.smallWhite{font-size:13px!important;opacity:.9}
...
.waSvg{width:18px;height:18px;display:block}
...
function render(){
  ...
  const tiles=calcDefs.map(def=>{const c=findCalc(def.engine);if(!c)return "";const meta=getScenarioMeta(c);return `<article class="calcTile" data-id="${esc(c.id)}" role="button" tabindex="0"><img src="${meta.image}" alt="${esc(def.label)}"><span class="title">${esc(def.label)}</span></article>`;}).join("");
  ...
  root.innerHTML=`<div class="page">
    ...
    <main>
      <section class="container topGrid">
        <article class="card adviceCard"><img class="heroImage" src="${heroImg}" alt="Asesoría financiera personalizada"><div class="adviceContent"><h1>${esc(heroTitle)}</h1><h2>${esc(heroSub)}</h2><p>Entendemos tu visión de éxito.</p><p>${esc(welcome)}</p><p>Analizamos tus necesidades actuales y futuras para ayudarte a construir un plan realista con visión de largo plazo.</p>${campaign?.publication_text?`<p class="smallWhite">${esc(campaign.publication_text)}</p>`:""}</div></article>
